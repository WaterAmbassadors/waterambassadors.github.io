...
(function () {
  ...
  const WAVES = [
    { yF:0.48, amp:30, f:0.006, spd:0.22, ph:0.0,  a:0.060 },
    { yF:0.57, amp:22, f:0.009, spd:0.16, ph:1.4,  a:0.042 },
    { yF:0.64, amp:16, f:0.013, spd:0.30, ph:2.7,  a:0.030 },
    { yF:0.71, amp:11, f:0.008, spd:0.13, ph:0.9,  a:0.020 },
    { yF:0.77, amp: 7, f:0.016, spd:0.38, ph:3.3,  a:0.013 },
  ];

  let W = 0, H = 0, t = 0, lastTs = null;
  ...
  function card(p) {
    ...
    img.onerror = function () {
      this.parentElement.innerHTML =
        `<span style="font-size:.7rem;color:var(--text-secondary);text-align:center;">${p.name}</span>`;
    };
    d.appendChild(img);
    return d;
  }

  // First set + identical clone → CSS moves the whole track by -50%
  PARTNERS.forEach(p => track.appendChild(card(p)));
  PARTNERS.forEach(p => track.appendChild(card(p)));
})();
...
(function () {
  ...
  if (dotsWrap) {
    POSTERS.forEach((_, i) => {
      ...
      b.addEventListener('click', () => { stopAuto(); scrollTo(i); startAuto(); });
      dotsWrap.appendChild(b);
    });
  }

  function allSlides() { return [...scroller.querySelectorAll('.poster-slide')]; }
  function allDots()   { return dotsWrap ? [...dotsWrap.querySelectorAll('.carousel-dot')] : []; }
  ...
  function scrollTo(idx) {
    ...
    const offset = target.offsetLeft - scroller.offsetLeft
  ...
  prevBtn?.addEventListener('click', () => { stopAuto(); scrollTo(current - 1); startAuto(); });
  nextBtn?.addEventListener('click', () => { stopAuto(); scrollTo(current + 1); startAuto(); });

  function startAuto() { autoTimer = setInterval(() => scrollTo(current + 1), 4500); }
  function stopAuto()  { clearInterval(autoTimer); }

  /* Init */
  requestAnimationFrame(() => { scrollTo(0); startAuto(); });
})();
...
(function () {
  ...
  function fillSlider(el, pct) {
    if (!el) return;
    el.style.background =
      `linear-gradient(to right, var(--ocean-glow) ${pct}%, rgba(255,255,255,0.12) ${pct}%)`;
  }
  ...
  audio.addEventListener('ended', () => {
    setPlaying(false);
    if (prog) { prog.value = 0; fillSlider(prog, 0); }
  ...
  audio.addEventListener('timeupdate', () => {
    ...
    if (prog) { prog.value = pct; fillSlider(prog, pct); }
